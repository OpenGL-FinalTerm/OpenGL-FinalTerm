...
#include "banana_draw.h"// ¾Ö ÀÌ»óÇÑÄ£±¸ÀÎµ¥......................... ´Ù¸¥°÷¿¡ ³ÖÀ¸¸é ¾Èµ¹´øµ¥....
#include "LoadMap.h"
#include "Character.h"
...
void S02Main::render()
{
	...
	if (opening_camera_working) {//¿ÀÇÁ´× ¿µ»óÀÌ ½ÃÀÛµÇ¸é
	...
	if (!opening_camera_working) { //¿ÀÇÁ´× Ä«¸Þ¶ó ¿öÅ·ÀÌ false
		//eye µµ °¢µµ¿¡ µû¶ó ¹Ù²ï´Ù.
	...
	if (person_view_1) {//1ÀÎÄª
		...
		//°¢µµ¿¡ ¸ÂÃç¼­ Ä«¸Þ¶ó¸¦ µ¹·ÁÁØ´Ù.
		//ÀÌ¶§ Ä«¸Þ¶óÀÇ At º¤ÅÍ ±âÁØÁ¡Àº eye°¡ ¾Æ´Ñ ¹Ù³ª³ªÀÇ ÇöÀç À§Ä¡ÀÔ´Ï´Ù.
...
*/
		...
		case 'u':
			...
			if (change_person_view_count % 2 == 0) {
				//view_rotate[0] => ÃÖ¼Ò ¹üÀ§ (20) Ä«¸Þ¶óÀÇ È¸Àü¹Ý°æÀ» ³ªÅ¸³À´Ï´Ù. 
				// view_rotate[1] => (20) Ä«¸Þ¶óÀÇ ³ôÀÌ¸¦ ³ªÅ¸³À´Ï´Ù. 
				view_rotate[0] = 60;
				view_rotate[1] = 50;

			}
			else if (change_person_view_count % 2 == 1) {         //3ÀÎÄª
		...
		case 'q':
			if (catchBox == false && pickLight == false) {
				if (result_degree[0] >= 135 && result_degree[0] < 225) { //¾Õ¿¡ÀÖ´Â°Å wPress
					for (int i = 0; i < whatBox; ++i) {
				...
				else if (result_degree[0] >= 225 && result_degree[0] < 315) { // ¿ÞÂÊ¿¡ ÀÖ´Â°Å aPress
					for (int i = 0; i < whatBox; ++i) {
				...
				else if ((result_degree[0] >= 315 && result_degree[0] < 360) || (result_degree[0] >= 0 && result_degree[0] < 45)) {  // µÚ¿¡ÀÖ´Â°Å sPress
					for (int i = 0; i < whatBox; ++i) {
						if (objectBox[i].returnBoxCenterX() - 10 < tmpRect.x + 5 && objectBox[i].returnBoxCenterX() + 10 > tmpRect.x - 5
							&& objectBox[i].returnBoxCenterZ() + 10 > tmpRect.z - 5 + 20 && objectBox[i].returnBoxCenterZ() - 10 < tmpRect.z + 5 + 20
							&& objectBox[i].returnBoxCenterY() - 10 < tmpRect.y + 5 && objectBox[i].returnBoxCenterY() + 10 > tmpRect.y - 5) {
				...
				else if (result_degree[0] >= 45 && result_degree[0] < 135) {  // ¿À¸¥ÂÊ¿¡ ÀÖ´Â°Å dPress
					for (int i = 0; i < whatBox; ++i) {
						if (objectBox[i].returnBoxCenterX() - 10 < tmpRect.x + 5 + 20 && objectBox[i].returnBoxCenterX() + 10 > tmpRect.x - 5 + 20
							&& objectBox[i].returnBoxCenterZ() + 10 > tmpRect.z - 5 && objectBox[i].returnBoxCenterZ() - 10 < tmpRect.z + 5
							&& objectBox[i].returnBoxCenterY() - 10 < tmpRect.y + 5 && objectBox[i].returnBoxCenterY() + 10 > tmpRect.y - 5) {
...
// banana pos add
	...
	// ¹Ú½º¿Í »¡°£ ±âµÕÀÌ Ãæµ¹(?) Ã¼Å© ÇØ¾ßÇÔ
	// ´ë·«ÀûÀÎ À§Ä¡ -> ¸ðµç Ãæµ¹ÀÌ ÀÏ¾î³­ÈÄ(È¤Àº ±×³É °ãÄ¡±â)
...
>>>>>>> parent of 6939309... ë˜ì§„ìƒìžì— ë‚‘ê¸°ëŠ” ë²„ê·¸ ìˆ˜ì •
					down = TRUE;
					jump = FALSE;
					tmpRect.jumpCount = 0;
					tmpRect.y -= 1;
	...
	//Á¶¸í ±×¸²ÀÚ shadowMapping

}

void S02Main::DefaultBoxPosSetting()
{
	// Scene01 Ä«¸Þ¶ó´Â ¾ðÁ¦Âë Á¦´ë·Î µ¹±î.....
	// ÀÌÁ¨ ÁøÂ¥ 6ÀÏ ³²¾Ò´Âµ¥....
	// ÀÌ»óÅÂ·Î °ÔÀÓÀº ³ª¿À±ä ÇÒ±î....
	// ÀÏ´Ü ¹ö¸®´Â ÇÔ¼ö´Â ÀÏ±âÀå
	// ÇÏ·ç¸¸ ´õ ¹Ï¾îº»´Ù.....................
